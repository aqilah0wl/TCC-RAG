...
class AIService:
    ...
    def __init__(self):
        ...
        self.model = OnnxEmbeddingModel(model_name)
    ...
    def semantic_search(self, query: str, min_similarity: float = 0.7, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Bulk calculate cosine similarity using pure NumPy
            query_vec = np.array(query_embedding).reshape(1, -1)
            embed_matrix = np.array(embeddings)
            # dot product / (norm_a * norm_b)
            dot_products = np.dot(embed_matrix, query_vec.T).flatten()
            norms = np.linalg.norm(embed_matrix, axis=1) * np.linalg.norm(query_vec)
            similarities = dot_products / np.clip(norms, 1e-9, None)
